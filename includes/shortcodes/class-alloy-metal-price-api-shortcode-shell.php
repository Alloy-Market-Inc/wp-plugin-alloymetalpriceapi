...
class Alloy_Metal_Price_API_Shortcode_Shell {
	 ...
	 * @return string
	 */
	public static function render($content, $skeleton, $prefix = 'alloy-shortcode') {
		$instance_id = wp_unique_id(sanitize_key($prefix) . '-');
		$skeleton_id = $instance_id . '-skeleton';

		return sprintf(
			'<div class="alloy-shortcode-shell">%1$s<div id="%2$s" class="alloy-shortcode-skeleton" aria-hidden="true">%3$s</div><div id="%4$s" class="alloy-shortcode-content" data-alloy-shortcode-pending="true">%5$s</div>%6$s</div>',
