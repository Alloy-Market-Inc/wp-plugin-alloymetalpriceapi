...
2. Open and merge a pull request into `release`.
3. At Wednesday 8:00 AM Central, the repository compares deployable files at the current merged SHA with the production-verified SHA and dispatches changed code to `Alloy-Market-Inc/wp-site-alloy-marcom`.
...
The command only accepts the clean current `origin/release` SHA and requires an associated merged release-branch PR. It is an emergency redispatch path; normal merged changes wait for the weekly window. It does not upload files.
